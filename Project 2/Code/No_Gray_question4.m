...
SNR_range = -4:2:20;         % SNR range from -4 to 20 in steps of 2 dB
...
for snr_idx = 1:length(SNR_range)
    ...
    detected_bits_matrix = de2bi(detected_symbols, k, 'left-msb');
...
xlabel('SNR (dB)');
ylabel('BER');
title(sprintf('BER vs. SNR for %d-PSK without Gray Coding', M));
legend('show');
grid on;
